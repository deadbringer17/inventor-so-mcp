...
public static class ServerInstructions
{
    public const string Text = "Inventor SO MCP for Inventor 2027. Query the active document and its revision before planning. " +
        ...
        "inventor_create_drawing_safe lays out a production drawing sheet: sheet size A4 to A0, landscape or portrait, first-angle (ISO/UNI) or third-angle (ANSI) projection, a chosen set of front/back/top/bottom/left/right/iso views, and an automatic ISO scale that leaves a dimensioning gutter around every view. It adds no dimensions and no parts list. " +
        "For a company title block, pass template: a drawing template file NAME from the host-owned template library, listed by inventor_list_drawing_templates, never a path. The template supplies the sheet, border and title block, so sheet_size and orientation are refused with it, and its sidecar manifest declares the area left free for views. Title-block text is filled by passing title_block_json, a JSON object of iProperty name to value, because an Inventor title block displays iProperties rather than sheet text. " +
